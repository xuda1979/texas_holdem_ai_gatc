...
from poker_ai.cli import train as run_training  # noqa: E402
...
class TestRunTraining(unittest.TestCase):
    ...
    @patch("time.time")
    def test_time_based_saving_triggered(
        self: Self,
        mock_time_time: MagicMock,
        mock_initialize_trainer: MagicMock,
    ) -> None:
        ...
        mock_time_time.side_effect = [
            ...
            # last_save_time becomes this value
            1000.0
            + save_interval_seconds
            + 1.0
            + 30.0,  # Call 4: Hand 3 check (30s elapsed from last_save_time) -> no save
        ]
        ...
        with (
            ...
            patch("poker_ai.cli.train.SelfPlay") as mock_self_play,
        ):  # Corrected patch target

            mock_self_play_instance = mock_self_play.return_value
    ...
    @patch("poker_ai.cli.train.initialize_trainer")
    @patch("time.time")
    def test_hand_based_saving_still_works(
        self: Self,
        mock_time_time: MagicMock,
        mock_initialize_trainer: MagicMock,
    ) -> None:
    ...
    @patch("poker_ai.cli.train.initialize_trainer")
    @patch("time.time")
    def test_no_saving_if_conditions_not_met(
        self: Self,
        mock_time_time: MagicMock,
        mock_initialize_trainer: MagicMock,
    ) -> None:
    ...
    @patch("poker_ai.cli.train.initialize_trainer")
    @patch("time.time")
    def test_time_based_saving_with_default_config(
        self: Self,
        mock_time_time: MagicMock,
        mock_initialize_trainer: MagicMock,
    ) -> None:
        # Tests if default time (10 min) is used if not in args
        # and specific value not in loaded config
        mock_trainer_instance = MagicMock()
        mock_trainer_instance.save_model = MagicMock()
        mock_initialize_trainer.return_value = mock_trainer_instance
