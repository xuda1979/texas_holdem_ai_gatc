...
class PokerGameGUI:
    ...
    def _sync_gui_with_engine_state(self):
        ...
        self.ai_money = (
            rules.player_chips[1]
            if hasattr(rules, "player_chips") and len(rules.player_chips) > 1
            else 0
        )
    ...
    def create_card_label(self, parent, card_name, show_back=False):
        ...
        if show_back or card_name not in self.card_images:
            ...
            if self.card_back_image:
                return tk.Label(
                    parent, image=self.card_back_image, bg=parent.cget("bg"), relief="solid", bd=1
                )
            else:
                # Fallback to text if no images available
                return tk.Label(
                    parent,
                    text=card_name if not show_back else "??",
                    font=("Arial", 10, "bold"),
                    bg="white",
                    fg="black",
                    width=8,
    ...
    def update_display(self):
        ...
        tk.Label(
            self.info_frame, text=ai_info, font=("Arial", 12), bg="darkgreen", fg="white"
        ).pack()
        ...
        tk.Label(
            self.player_frame, text=call_text, font=("Arial", 12), bg="darkblue", fg="yellow"
        ).pack()
    ...
    def request_human_action(self, game, player_index, amount_to_call):
        ...
        self.status_label.config(
            text=f"Your turn! Pot: ${game.rules.pot} | Your chips: ${game.rules.player_chips[0]}"
        )
            ...
            if max_raise < min_raise:
                ...
                if amount_to_call > 0:
                    messagebox.showwarning(
                        "Cannot Raise",
                        "You don't have enough chips to raise. You can only call or fold.",
                    )
                else:
                    messagebox.showwarning(
                        "Cannot Bet", "You don't have enough chips to bet. You can only check."
                    )
                return
