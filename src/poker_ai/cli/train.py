...
# If script is not in root, adjust path as needed. For an absolute path or
# environment variable, modify CONFIG_FILE_PATH accordingly.
...
def main() -> None:  # noqa: C901
    ...
    save_model_every_seconds = (
        save_model_every_minutes * 60
        if save_model_every_minutes and save_model_every_minutes > 0
        else None
    )
    ...
    for iteration in range(1, num_iterations + 1):
        ...
        # Hand-based checkpointing
        if save_model_every_hands and save_model_every_hands > 0:
            if iteration % save_model_every_hands == 0:
                cfr_trainer.save_model(f"models/{args.algorithm}_autosave.pth")

        # Time-based checkpointing
        if save_model_every_seconds is not None:
            current_time = time.time()
            if current_time - last_save_time >= save_model_every_seconds:
                cfr_trainer.save_model(f"models/{args.algorithm}_autosave.pth")
