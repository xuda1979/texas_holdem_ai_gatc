...
\begin{abstract}
We present Belief--MCTS--\sCRF as a design study for a multi\textendash player No\textendash Limit Hold'em agent that couples a transformer\textendash style public encoder, a street\textendash structured CRF (\sCRF) opponent model, belief\textendash aware Monte Carlo tree search, and lightweight endgame re\textendash solving via CFR$^+$. The paper focuses on methodological and theoretical contributions rather than benchmark performance. Our main technical components are: (i) Consistency\textendash Projected Beliefs (CPB), a maximum\textendash entropy projection onto a realizable, group\textendash invariant family used to generate admissible root\textendash chance beliefs; (ii) Leakage\textendash Free Public\textendash Subgame CFR$^+$ with a symmetry\textendash invariance guarantee under CPB; and (iii) IS\textendash RS\textendash CVaR\textendash UCT, which combines information\textendash set root sampling, a streaming Rockafellar--Uryasev CVaR estimator, and bandit\textendash gated double progressive widening for continuous bet sizing. We derive strong convexity and conditioning bounds for CPB, including a reference\textendash metric whitening view; establish selection\textendash stability guarantees for CVaR\textendash UCT; and prove almost\textendash sure densification guarantees for the widening schedule. Empirically, we report only deterministic toy diagnostics generated by the embedded simulation script and do not claim competitive poker performance. The manuscript is therefore best read as a mathematically explicit blueprint for future end\textendash to\textendash end implementations and benchmark studies.
\end{abstract}
...
\begin{itemize}
\item \sCRF\ opponent modeling with a single trajectory\textendash wide base measure and consistent cross\textendash street bucketization for calibrated posterior updates.
\item IS\textendash RS\textendash CVaR\textendash UCT, a belief\textendash aware MCTS selection rule with a streaming RU CVaR estimator and bandit\textendash gated double progressive widening for continuous actions.
\item Leakage\textendash Free Public\textendash Subgame CFR$^+$ that uses CPB as root chance and preserves public\textendash state symmetries.
\item An accelerated CPB analysis with fixed diagonal preconditioning, reference\textendash metric whitening, and continuation, while retaining a simple deterministic GD baseline for diagnostics.
\item Transparent deterministic diagnostics, rather than benchmark claims, covering CPB projection residuals, IS\textendash RS unbiasedness under CRN, RU\textendash CVaR convergence on Uniform$[-1,1]$, and DPW densification.
\end{itemize}
...
\small
\textbf{Algorithm 1: Preconditioned--Accelerated CPB (single $\gamma$ stage).}
\begin{enumerate}
\item \textbf{Input:} standardized statistics $\{S_o\}$, target $\widehat s$, penalty $\gamma>0$, tolerance $\varepsilon$, radius $R$, and fixed diagonal preconditioner $D_{\mathrm{ref}}$.
\item Initialize $\lambda^{(0)}=0$, $y^{(0)}=\lambda^{(0)}$, $\theta^{(0)}=1$, and stepsize $\eta\gets 1/(R^2+\gamma)$.
\item For $t=0,1,2,\ldots$, compute scores $u_o=\langle y^{(t)},S_o\rangle$, weights $w_o \propto \exp(u_o-\max_o u_o)$, and partition function $Z=\sum_o w_o$.
\item Form $\E[S]\gets \sum_o \frac{w_o}{Z}S_o$ and gradient $g^{(t)}\gets \E[S]-\widehat s + \gamma y^{(t)}$.
\item Update $\lambda^{(t+1)}\gets y^{(t)}-\eta D_{\mathrm{ref}}g^{(t)}$.
\item Update the acceleration parameter $\theta^{(t+1)}\gets \tfrac{1+\sqrt{1+4(\theta^{(t)})^2}}{2}$ and extrapolated point
\[
y^{(t+1)}\gets \lambda^{(t+1)}+\frac{\theta^{(t)}-1}{\theta^{(t+1)}}\bigl(\lambda^{(t+1)}-\lambda^{(t)}\bigr).
\]
\item Stop when both $\|\nabla f_\gamma(\lambda^{(t+1)})\|_2 \le \varepsilon$ and $\|\E_{q_{\lambda^{(t+1)}}}[S]-\widehat s\|_2\le 10^{-3}$ hold; optionally restart if $f_\gamma(\lambda^{(t+1)})>f_\gamma(\lambda^{(t)})$.
\item Return $\lambda^\star\gets \lambda^{(t+1)}$.
\end{enumerate}
\end{minipage}
\caption{Portable pseudocode for the analyzed CPB solver.}
\label{alg:cpb}
\end{figure}

\subsection{\texorpdfstring{Leakage\textendash Free Public\textendash Subgame CFR$^+$}{Leakage-Free Public-Subgame CFR+}}
...
\begin{minipage}{0.96\linewidth}
\small
\textbf{Two\textendash timescale RU CVaR tracker (per edge).}

\textbf{Input:} tail level $\alpha$, steps $a_n,b_n$, and projections $\Pi_t,\Pi_m$.

\textbf{Initialize:} $t_0=0$ and $m_0=0$.

\textbf{For} $n=1,2,\dots$:
\begin{enumerate}
\item Observe loss $Y_n\in[-1,1]$.
\item Update
\[
g_t \gets 1-\frac{1}{1-\alpha}\1\{Y_n>t_{n-1}\},\qquad
t_n\gets \Pi_t\bigl(t_{n-1}-a_n g_t\bigr).
\]
\item Set
\[
u_n\gets \max\{0,Y_n-t_n\},\qquad
m_n\gets \Pi_m\bigl(m_{n-1}+b_n(u_n-m_{n-1})\bigr).
\]
\item Output
\[
\widehat{\CVaR}_{\alpha,n}\gets \mathrm{clip}\bigl(t_n+\frac{m_n}{1-\alpha},-1,1\bigr).
\]
\end{enumerate}
\end{minipage}%
}
...
\bottomrule
\end{tabular}
\caption{CPB toy diagnostics. Residuals decrease as the penalty $\gamma$ is reduced on the orbit\textendash completed toy support.}
\label{tab:cpb}
\end{table}
...
\toprule
Mean (no resample) & Mean (resample) & 95\% CI half-width & Mean diff & 95\% CI (diff) & Sims \\
\midrule
0.001 & 0.001 & 0.000160 & 0.000000 & 0.000000 & 20000 \\
\bottomrule
\end{tabular}
\caption{IS\textendash RS expected returns with paired CRN. The difference interval collapses to zero in this deterministic sanity check.}
\label{tab:isrs}
\end{table}

\subsection{\texorpdfstring{Streaming RU CVaR estimator on Uniform$[-1,1]$}{Streaming RU CVaR estimator on Uniform[-1,1]}}
...
\begin{tabular}{ccccc}
\toprule
$n$ & $\widehat{\CVaR}_{0.90}$ & Abs. err. & $\widehat{\CVaR}_{0.95}$ & Abs. err. \\
\midrule
100 & 0.780 & 0.120 & 0.840 & 0.110 \\
300 & 0.850 & 0.050 & 0.900 & 0.050 \\
1000 & 0.890 & 0.010 & 0.930 & 0.020 \\
3000 & 0.905 & 0.005 & 0.948 & 0.002 \\
10000 & 0.900 & 0.000 & 0.951 & 0.001 \\
\bottomrule
\end{tabular}
\caption{Streaming RU CVaR estimates on Uniform$[-1,1]$ losses. The estimates approach the ground\textendash truth values $\CVaR_{0.90}=0.90$ and $\CVaR_{0.95}=0.95$ as the sample count grows.}
\label{tab:cvar}
\end{table}
...
\bottomrule
\end{tabular}
\caption{DPW coverage diagnostics. Accepted size count grows with node visits while the minimum gap shrinks toward zero.}
\label{tab:dpw}
\end{table}

\subsection{CVaR\textendash UCT selection with DPW gating}
We formalize a risk\textendash sensitive selection rule that combines double progressive widening with an independent forced\textendash acceptance coin.
...
\begin{minipage}{0.96\linewidth}
\small
\textbf{Algorithm 1: CVaR\textendash UCT with DPW gating (single node $x$).}
\begin{enumerate}
\item \textbf{Input:} node $x$, prior $\tilde P(\cdot\mid x)$, weight $\lambda\ge 0$, tail level $\alpha\in(0,1)$, exploration constant $c_{\text{puct}}$, forced\textendash accept schedule $\varepsilon_n$, and proposal sampler $\mathcal{Q}_x$.
\item If $\mathrm{should\_propose}(N(x))$ is true, propose $a\sim\mathcal{Q}_x$ and accept it if either $\mathrm{Bern}(\varepsilon_{N(x)})=1$ or $\mathrm{bandit\_accept}(x,a)$ is true.
\item For each child $(x,a)$, compute
\[
U(x,a)=\hat\mu_{x,a}-\lambda\,\widehat{\CVaR}_\alpha(Y_{x,a}) + c_{\text{puct}}\,\tilde P(a\mid x)\,\frac{\sqrt{\sum_b N(x,b)}}{1+N(x,a)}.
\]
\item Select
\[
a^\star\in\arg\max_a U(x,a),
\]
breaking ties by larger $\tilde P(a\mid x)$.
\item Return $a^\star$.
\end{enumerate}
\end{minipage}%
}
\caption{CVaR\textendash UCT with DPW gating at a single search node.}
...
\begin{itemize}
\item One authoritative code file: this paper embeds a single \texttt{simulation.py} that deterministically writes \texttt{results.txt}, \texttt{cpb.csv}, \texttt{isrs.csv}, \texttt{rucvar\_090.csv}, \texttt{rucvar\_095.csv}, and \texttt{dpw.csv}.
\item Unified schemas: all CSV schemas are exactly those consumed by the LaTeX tables and figures.
\item Byte identity: the CSV files and \texttt{results.txt} embedded via \texttt{filecontents*} are byte\textendash identical copies of the artifacts created by \texttt{simulation.py}.
\item Reproduction: run \texttt{python3 simulation.py} and then compile the manuscript with \texttt{pdflatex}.
\item NLHE normalization: any future poker experiments should normalize chip payoffs as a clipped stack fraction, $X=\mathrm{clip}(\Delta/S_{\mathrm{norm}},-1,1)$ with $S_{\mathrm{norm}}=\min\{S_{\mathrm{eff}},200\ \text{bb}\}$, preserving the bounded\textendash loss assumptions used by the CVaR analysis.
\item Scope: we do not report live or simulated NLHE benchmark outcomes; the embedded diagnostics are deterministic sanity checks only.
\item Environment: Python 3.8+ with standard library only; no third\textendash party dependencies.
\end{itemize}
...
\begin{itemize}
\item Stable bucket mapping across streets: boundaries are defined in pot\textendash fraction with left\textendash closed, right\textendash open intervals (last bucket closed). Historical sizes are mapped by pot fraction at action time, so the mapping remains order preserving.
\item Selection stability under parameter damping: if the CVaR level $\alpha$ and weight $\lambda$ change by at most $\delta_\alpha,\delta_\lambda$ per $k$ visits, the induced score drift per update is bounded by $|\Delta U|\le \delta_\lambda + \frac{2}{(1-\alpha)^2}\delta_\alpha$. Choosing damping with $|\Delta U|<\tfrac{1}{2}\Delta$ preserves action ordering by the stability bound.
\item SA with Markov noise: conditional on a fixed menu or policy, edge\textendash local losses form an ergodic Markov chain. Nonvanishing exploration and reintroduction therefore imply infinitely many visits almost surely, satisfying the standard stochastic\textendash approximation visitation conditions \cite{borkar2008sa,kushner2003sa}.
\item CPB diagnostics vs.\ production: the diagnostics use diminishing\textendash step GD (200 iterations) for transparency and determinism. The analyzed accelerated solver combines whitening (\cref{lem:refwhiten}), a fixed diagonal preconditioner (\cref{prop:accel}), and continuation (\cref{prop:continuation}); optional adaptive restarts or RMS heuristics may help in practice but fall outside the stated guarantees.
\item Practical $\gamma$ selection: estimate local curvature $\mu_0\approx \lambda_{\min}(\mathrm{Cov}_{q_{\lambda}}(S))$ and $\|\lambda\|$ at a coarse $\gamma$, then choose the terminal $\gamma_T$ using \cref{prop:continuation} to meet a target moment\textendash bias tolerance.
\end{itemize}
