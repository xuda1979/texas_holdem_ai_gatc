...
def test_game_engine():
    ...
    try:
        ...
        for i in range(3):
            ...
            # Check deck has reasonable cards remaining
            # (should be 52 - 4 hole - 5 community - 3 burn = 40)
            if len(game.rules.deck) < 30:
                print(
                    f"✗ Deck exhaustion risk: only {len(game.rules.deck)} cards remaining after hand {i+1}"
                )
                return False
