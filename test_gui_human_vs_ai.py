...
def test_human_vs_ai_setup():
    ...
    for ai_count, total_players in test_cases:
        ...
        assert (
            len(player_strategies) == total_players
        ), f"Expected {total_players} strategies, got {len(player_strategies)}"
...
def test_invalid_setups():
    ...
    for ai_count, total_players in invalid_cases:
        ...
        if not (2 <= total_players <= 10):
            print("✓ Correctly rejected: Total players must be between 2 and 10")
            continue
        ...
        if total_players != ai_count + 1:
            print("✓ Correctly rejected: Total players must equal AI count + 1 human player")
            continue
...
def test_turn_management():
    ...
    game_engine = TexasHoldem(
        num_players=3, starting_stack=1000, player_strategies=player_strategies
    )
    ...
    print(
        f"Current player: {current_player}, Human index: {human_player_index}, Is human turn: {is_human_turn}"
    )
    ...
    for i in range(1, 3):
        if game_engine.player_strategies[i]:
            ...
            ), f"Player {i} should have RandomAIStrategy"
