...
def test_gui_human_strategy():
    ...
    class MockGUI:
        ...
        def request_human_action(self, game, player_index, amount_to_call):
            print(
                f"GUI callback called for player {player_index}, amount to call: {amount_to_call}"
            )
...
def test_game_setup_compatibility():
    ...
    print(
        f"✓ Game setup: {total_players} players, {num_humans} human, {num_ai} AI, ${starting_stack} stack"
    )
...
def test_gui_game_creation():
    ...
    print(
        f"✓ Player 0: {type(game.player_strategies[0]).__name__} (human: {game.player_strategies[0].is_human})"
    )
    for i in range(1, total_players):
        print(
            f"✓ Player {i}: {type(game.player_strategies[i]).__name__} (human: {game.player_strategies[i].is_human})"
        )
