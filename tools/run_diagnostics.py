...
from poker_ai.logging_utils import (  # noqa: E402
    log_configuration_snapshot,
    log_run_metadata,
    setup_logging,
)
from poker_ai.selfplay.self_play import SelfPlay  # noqa: E402
from poker_ai.systems.catalog import recommend_validation_plan  # noqa: E402
...
def _load_train_cli_module():
    return importlib.import_module("poker_ai.cli.train")


def _resolve_model(trainer: Any) -> Any:
    model = getattr(trainer, "model", None)
    if model is not None:
        return model
    model = getattr(trainer, "advantage_net", None)
    if model is not None:
        return model
    raise AttributeError("Trainer does not expose a model or advantage_net")
...
def _diagnose_forward_pass(trainer: Any, logger: logging.Logger) -> dict[str, float]:
    ...
    card_dim = int(getattr(trainer, "card_feature_dim", getattr(card_projection, "in_features", 32)))
    history_dim = int(getattr(trainer, "history_feature_dim", getattr(history_projection, "in_features", card_dim)))
    seq_len = getattr(trainer, "max_seq_len", getattr(model, "max_seq_len", None))
    model_cfg = {}
    ...
    hole = torch_module.zeros(1, card_dim)
    community = torch_module.zeros(1, card_dim)
    history = torch_module.zeros(1, seq_len, history_dim)

    start = time.perf_counter()
    with torch_module.no_grad():
        _ = model(hole, community, history)
...
def _diagnose_training_step(trainer: Any, logger: logging.Logger) -> dict[str, float]:
    ...
    if isinstance(getattr(trainer, "config", None), dict):
        model_cfg = trainer.config.get("model", {})
    seq_len = int(model_cfg.get("max_seq_len", getattr(trainer, "max_seq_len", getattr(model, "max_seq_len", 16))))
    num_actions = int(getattr(trainer, "num_actions", getattr(model, "num_actions", 0)))
    legal_mask = torch_module.ones(num_actions, dtype=torch_module.bool)
    hole = torch_module.zeros(card_dim)
    community = torch_module.zeros(card_dim)
    history = torch_module.zeros(seq_len, history_dim)
    action_values = torch_module.randn(num_actions)

    start = time.perf_counter()
    add_experience = getattr(trainer, "add_experience", None)
    if callable(add_experience):
        add_experience(
            hole,
            community,
            history,
        ...
        loss = trainer.train(batch_size=1)
    else:
        loss = trainer.train(
            "diagnostic",
            hole,
...
def _diagnose_self_play(trainer: Any, config: dict[str, Any], logger: logging.Logger) -> dict[str, float]:
    ...
    training_cfg["train_during_generation"] = False
    training_cfg["min_buffer_before_train"] = max(
        int(training_cfg.get("min_buffer_before_train", 256)),
        256,
    )
...
def build_iteration_plan(changed_paths: list[str]) -> dict[str, Any]:
    return recommend_validation_plan(changed_paths)


def run_targeted_tests(test_targets: list[str], runner=subprocess.run) -> int:
...
def main() -> int:
    train_cli = _load_train_cli_module()
    args = parse_args()
    config = train_cli.load_configuration(args.config)
    ...
    try:
        trainer = train_cli.initialize_trainer(args.algorithm, config, device="cpu")
